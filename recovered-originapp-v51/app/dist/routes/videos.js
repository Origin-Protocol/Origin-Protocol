...
async function handleSealedPublish(req, res) {
    ...
    const video = {
        ...
        description: verifyResult
            ? (0, videoVerificationService_1.appendOriginVerificationMetadata)(payload.description, verifyResult)
...
router.post('/cloudflare/finalize', auth_1.requireAuth, async (req, res) => {
    ...
    const verificationDescription = creatorVerification.verifyResult
        ? (0, videoVerificationService_1.appendOriginVerificationMetadata)(payload.description, creatorVerification.verifyResult)
        : payload.description;
    const now = new Date().toISOString();
    const id = (0, uuid_1.v4)();
    const video = {
...
router.post('/', auth_1.requireAuth, upload.single('video'), async (req, res) => {
    ...
    const { title, description, originBundleId, assetId, keyId, protectedUpload, contentHash, originId } = parsed.data;
    ...
    const verifiedDescription = originVerifyResult
        ? (0, videoVerificationService_1.appendOriginVerificationMetadata)(description, originVerifyResult)
        : description;
