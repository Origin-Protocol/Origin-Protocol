...
def _build_urls(ledger_cid: str, upstreams: Iterable[str]) -> list[str]:
    ...
    for upstream in upstreams:
        ...
        elif "/ledger/" in upstream or "/ipfs/" in upstream:
...
def discover_ledger(base_url: str, ledger_kind: str) -> tuple[str | None, list[str]]:
    base = base_url.rstrip("/")
    payload: dict[str, Any] | None = _fetch_json(f"{base}/ledger")
    if not payload:
        return None, [base]

    ledgers: Any = payload.get("ledgers")
    if not isinstance(ledgers, list):
        return None, [base]

    preferred: str | None = None
    fallback: str | None = None
    entries = cast(list[Any], ledgers)
    for item in entries:
        ...
        if isinstance(cid, str) and cid.strip():
            if fallback is None:
                fallback = cid.strip()
            if isinstance(kind, str) and kind.strip().lower() == ledger_kind:
                preferred = cid.strip()
                break

    cid = preferred or fallback
    if not cid:
        return None, [base]
    return cid, [f"{base}/ledger/{cid}", f"{base}/ipfs/{cid}", base]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Origin server base URL for auto-discovery (set empty to disable)",
    )
    parser.add_argument(
        "--ledger-kind",
        default="license",
        choices=("license", "governance"),
        help="Preferred ledger kind when auto-discovering from origin server",
    )
...
class LedgerService:
    def __init__(self, ledger_path: Path, ledger_cid: str | None, upstreams: list[str], ledger_kind: str) -> None:
...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        service = cast(LedgerService, getattr(self.server, "service"))
        if self.path.startswith("/health"):
            payload: dict[str, Any] = {
                "status": "ok",
                "ledger_cid": service.ledger_cid,
                "ledger_kind": service.ledger_kind,
...
def main() -> int:
    ...
    if not upstreams and args.origin_server.strip():
        discovered_cid, discovered_upstreams = discover_ledger(
            args.origin_server.strip(),
            args.ledger_kind.strip().lower(),
        )
        if discovered_upstreams:
            upstreams = discovered_upstreams
        if not ledger_cid:
            ledger_cid = discovered_cid

    service = LedgerService(
        ledger_path=ledger_path,
        ledger_cid=ledger_cid,
        upstreams=upstreams,
        ledger_kind=args.ledger_kind.strip().lower(),
    )
