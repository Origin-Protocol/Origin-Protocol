...
def _fetch_remote_ledger(full_node_url: str) -> bytes | None:
    base = full_node_url.rstrip("/")
    for url in (f"{base}/ledger", f"{base}/ipfs"):
        try:
...
def _load_state(path: Path, public_key_override: str | None) -> dict:
    ...
    try:
        ...
        try:
            plain_ledger = json.loads(raw.decode("utf-8"))
        except Exception:
            plain_ledger = None

    return {
        "exists": True,
        "format": "plain_json" if plain_ledger is not None else "unknown",
        "ledger": plain_ledger,
        "signature": None,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Path to node ledger JSON (defaults to full_node_service path)",
    )
    parser.add_argument("--public-key", default="", help="Public key PEM to verify ledger signature")
    parser.add_argument(
        "--full-node-url",
        default="",
        help="Optional full-node URL to pull ledger from if local ledger file is missing",
    )
    parser.add_argument(
        "--require-valid",
        action="store_true",
...
class RegistryService:
    ...
    def load_state(self) -> dict:
        if self.full_node_url and not self.ledger_path.exists():
            remote = _fetch_remote_ledger(self.full_node_url)
            if remote:
                self.ledger_path.parent.mkdir(parents=True, exist_ok=True)
                self.ledger_path.write_bytes(remote)
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if self.path.startswith("/nodes"):
            ...
            if state["format"] == "node_ledger":
                ...
                payload = {
                    "format": state["format"],
                    "entries": len(ledger.get("entries", [])) if isinstance(ledger, dict) else 0,
                    "authorities": state["authorities"],
                    "keys": sorted(list(ledger.keys())) if isinstance(ledger, dict) else [],
                }
            self._send_json(payload)
            return
...
def main() -> int:
    ...
    service = RegistryService(
        ...
        full_node_url=args.full_node_url.strip() or None,
    )
