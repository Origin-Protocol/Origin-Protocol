...
function stripMarker(description, marker) {
    if (!description)
        return description;
    const markerIndex = description.indexOf(marker);
    if (markerIndex < 0)
        return description;
    return description.slice(0, markerIndex).trimEnd() || null;
}
function extractOriginProtectionMeta(description) {
    if (!description) {
        return { originPolicy: null, governanceLedgerCid: null };
    }
    const markerIndex = description.lastIndexOf(ORIGIN_PROTECTION_MARKER);
    if (markerIndex < 0) {
        return { originPolicy: null, governanceLedgerCid: null };
    }
    const encoded = description.slice(markerIndex + ORIGIN_PROTECTION_MARKER.length).trim();
    if (!encoded) {
        return { originPolicy: null, governanceLedgerCid: null };
    }
    try {
        const parsed = JSON.parse(encoded);
        const policy = typeof parsed?.policyProfile === 'string' ? parsed.policyProfile : null;
        const governanceLedgerCid = typeof parsed?.governanceLedgerCid === 'string' ? parsed.governanceLedgerCid : null;
        return {
            originPolicy: policy,
            governanceLedgerCid,
        };
...
function toVideoMeta(video) {
    ...
    const protection = extractOriginProtectionMeta(meta.cleanDescription);
    const withoutProtection = stripMarker(meta.cleanDescription, ORIGIN_PROTECTION_MARKER);
    const withoutFingerprint = stripMarker(withoutProtection, ORIGIN_FINGERPRINT_MARKER);
    return {
        ...video,
        description: withoutFingerprint,
        originPolicy: video.originPolicy ?? protection.originPolicy ?? null,
        governanceLedgerCid: video.governanceLedgerCid ?? protection.governanceLedgerCid ?? null,
