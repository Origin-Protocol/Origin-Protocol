...
## Origin Main node asset sync

If you keep canonical protocol assets in `Origin Main/platform`, copy these into
`experimental/server/ledgers` before running or deploying the server:

- `bootstrap_v1.json`
- `governance_v1.json`
- `governance_v1.cid`

Optional but recommended to keep alongside node assets:

- `platform_bootstrap.json`
- `platform_policies.json`

The recovered node endpoints in this server expose those ledgers via:

- `GET /ledger`
- `GET /ledger/:cid`
- `GET /ipfs/:cid`

Override locations with environment variables when needed:

- `ORIGIN_LICENSE_LEDGER_PATH`
- `ORIGIN_GOVERNANCE_LEDGER_PATH`
- `ORIGIN_GOVERNANCE_LEDGER_CID_PATH`
